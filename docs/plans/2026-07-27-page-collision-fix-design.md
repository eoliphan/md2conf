...
> **CORRECTION (2026-07-28, found during implementation and reproduced empirically).** The claim below holds **only when the two efforts pass different `-r` anchors.** If both efforts pass the *same* anchor and neither root document carries an embedded page id, Effort B's root page genuinely *is* a descendant of the shared anchor — so Stage 1 passes, Effort A's managed root becomes Effort B's root page, and A publishes its tree inside B's, overwriting B's root content. Hierarchical containment cannot express ownership when the hierarchy is shared. Closing it needs an independent ownership signal (a page property naming the owning effort), not a deeper tree walk. See the plan's Known Gaps; **`-r` must be per-effort, and that is now a correctness requirement, not a recommendation.**
